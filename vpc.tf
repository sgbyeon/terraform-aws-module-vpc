...
resource "aws_subnet" "this" {
  ...
  cidr_block = var.subnets[each.value].cidr[item]
  availability_zone = var.azs[index(var.subnets[each.value].cidr, var.subnets[each.value].cidr[item])]

  tags = merge(var.tags, tomap({ Name = format("%s-%s-%s-%s-%s-sn", 
                                                var.prefix,
                                                var.vpc_name,
                                                var.azs[index(var.subnets[each.value].cidr, var.subnets[each.value].cidr[item])],
